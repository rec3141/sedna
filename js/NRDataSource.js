...
NRDataSource.prototype.load = function(data) {
    ...
    for (var x = 0; x < data.length; x++) {
        ...
        for (var i = 0; i < coordinates.length; i += 4) {
			...
			var normabundance = Math.sqrt( (1 + abundance - minabund)/(maxabund - minabund) );
			var normsize = minradius + (maxradius-minradius)*normabundance;
...
//            var color = Cesium.Color.fromHsl((0.6 - (normabundance * 0.5)), 1.0, 0.5);
			...
			var points = new Cesium.PointPrimitive({
